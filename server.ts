...
// Local offline fallback database simulator (Tailored perfectly to Iranian auto market)
function generateOfflineCarParts(carModel: string, specifications?: string) {
  ...
  const summary = isForeign 
    ? `بررسی فنی خودروی ${normalizedCarName} نشان می‌دهد این خودرو با توجه به پیشرانه مدرن و حساس خود، نیاز مبرمی به سرویس‌های منظم دوره‌ای مخصوصاً تعویض به‌موقع روغن‌های تمام‌سنتتیک دارد. به سبب استفاده قطعات خارجی، هزینه‌های استهلاک و لوازم یدکی آن در بازار چراغ‌برق تهران نسبتاً بالاست ولی عمر مفید قطعات طولانی‌تر است. توصیه می‌شود حتماً از برندهای جنیون پارت یا کره ای/آلمانی معتبر استفاده شود.`
    : `خودروی ${normalizedCarName} دارای استهلاک متداول بوده و دسترسی به قطعات یدکی آن در سراسر ایران بسیار آسان و با قیمت‌های رقابتی است. به دلیل کیفیت متوسط قطعات شرکتی، توصیه می‌شود سرویس‌های پنج هزارتایی روغن و فیلترها به موقع انجام شود تا عمر مفید موتور حفظ گردد. برندهای ایساکو، سایپا یدک و مارک‌های تحت پوشش در بازار چراغ‌برق تهران انتخاب‌های مطمئنی هستند.`;
      ...
      parts: [
        {
          ...
          priceRange: isForeign ? { min: 1400000, max: 2800000 } : { min: 380000, max: 680000 },
          lifetime: "۸,۰۰۰ کیلومتر یا ۶ ماه",
          replacementSigns: "سیاه شدن شدید مایع، صدای خشن سوپاپ‌ها هنگام استارت سرد، کاهش عملکرد",
        ...
        },
        {
          ...
          priceRange: isForeign ? { min: 450000, max: 950000 } : { min: 180000, max: 320000 },
          ...
          proTip: "روغن گیربکس اتوماتیک باید با دستگاه مخصوص تخلیه کامل شود تا لجن‌های پشت شیربرقی‌ها کاملاً خارج شوند."
        }
      ]
    },
    {
      ...
      parts: [
        {
          ...
          priceRange: isForeign ? { min: 1200000, max: 3500000 } : { min: 280000, max: 620000 },
        ...
        },
        {
          ...
          priceRange: isForeign ? { min: 2500000, max: 6500000 } : { min: 750000, max: 1600000 },
      ...
      parts: [
        {
          ...
          priceRange: isForeign ? { min: 1200000, max: 3000000 } : { min: 280000, max: 680000 },
        ...
        },
        {
          ...
          priceRange: isForeign ? { min: 1500000, max: 2800000 } : { min: 900000, max: 1600000 },
      ...
      parts: [
        {
          ...
          priceRange: isForeign ? { min: 180000, max: 480000 } : { min: 45000, max: 120000 },
          ...
          installDifficulty: "آسان (توسط راننده)",
          proTip: "هرگز فیلتر هوا را باد نگیرید! فیلتر بادخورده گرد و غبار میکرونی را مستقیماً وارد سیلندر موتور می‌کند."
        },
        {
          partName: "فیلتر کابین (تهویه مطبوع کربن فعال)",
          priceRange: isForeign ? { min: 150000, max: 420000 } : { min: 40000, max: 95000 },
          ...
          installDifficulty: "آسان (توسط راننده)",
          proTip: "فیلترهای کابین کربن فعال قدرت مهار بوهای نامطبوع محیط اطراف و بنزین جاده دارند."
        }
      ]
    },
    {
      ...
      parts: [
        {
          ...
          priceRange: isForeign ? { min: 2500000, max: 5500000 } : { min: 450000, max: 1200000 },
...
function generateOfflineChatReply(carModel: string, partName: string, userMessage: string): string {
  ...
  if (normMsg.includes("اجرت") || normMsg.includes("دستمزد") || normMsg.includes("تعویض")) {
    return `سلام دوست من! درباره دستمزد و اجرت تعویض قطعه در خودروی «${carModel}» پرسیدی. 
بر اساس آخرین نرخ‌نامه‌ی مصوب صنف مکانیک‌های ایران (سال ۱۴۰۵)، اجرت تعمیرات دوره‌ای به این صورته:
۱. تعویض لنت جلو: حدود ۱۵۰ تا ۲۵۰ هزار تومان
۲. تعویض روغن و تمامی فیلترها (سرویس کامل): حدود ۱۰۰ تا ۱۸۰ هزار تومان
۳. تعویض تسمه تایم و بلبرینگ‌ها: حدود ۷۰۰ تا ۱,۲۰۰,۰۰۰ تومان (برای خودرو‌های وارداتی ممکنه تا ۳ میلیون تومان هم برسه چون جاش بددست و ظریفه!)
موقع تعویض حتماً به اتومکانیک بفرمایید دیسک چرخ جلو را چک کند تا لبه عمیق نداشته باشد. دمت گرم، باز اگر سوالی داری در خدمتم!`;
  }
  
  if (normMsg.includes("خرید") || normMsg.includes("دیجی") || normMsg.includes("دیجیکالا") || normMsg.includes("اصل") || normMsg.includes("تقلبی")) {
    return `مخلصیم سالار! برای خرید قطعات خودروی «${carModel}» مخصوصاً قطعه کارآمد «${partName || "لوازم مصرفی"}» دو سه تا فوت‌و‌فن فنی رو از اوس عماد به یادگار داشته باش:
اولاً برای استعلام قیمت آنلاین، بهترین و مطمئن‌ترین جا سایت دیجی‌کالا (digikala.com) هست که مدل دقیق ماشینت رو سرچ کنی و نظرات خریداران رو بخونی. اگر اونجا موجود نداشت، حتماً از موتور جستجوی ترب (torob.com) یا یدک‌کالا و یدک‌یاب قیمت کالا رو بگیری.
ثانیاً موقع خرید حضوری در چراغ‌برق لوازم برقی حتماً بگو دارای شناسه رهگیری وزارت صمت باشند و بارکد رو توی گوشی چک کن.
ثالثاً اختلاف قیمت خیلی شدید را جدی بگیرید؛ وقتی همه یک مارک را ۲ تومن می‌فروشند و یکی به بهانه «خرید قدیم» می‌دهد ۹۰۰ هزار تومن، شک ننما احتمالاً قطعه بازسازی شده یا تقلبی چسب‌خورده است!`;
  }

  if (normMsg.includes("روغن") || normMsg.includes("لیکومولی") || normMsg.includes("بهران") || normMsg.includes("کاسترول")) {
    return `به به! استاد روغن موتور پرسیده. واسه خودروی «${carModel}» روغن با گرانروی مناسب خیلی حیاتیه رفیق. 
اگر موتور ماشین معمولی و داخلیه (مثل خانواده پژو یا ساینا) روغن 10W-40 نیمه‌سنتتیک عالی جواب میده (مثلاً بهران سوپر پیشتاز طلایی یا اسپیدی).
اما اگر موتور توربوشارژ، حساس یا وارداتی جدید داری، حتماً روغن تمام‌سنتتیک 5W-30 یا 5W-40 بریز (مثل کاسترول مگناتک یا ادینول آلمانی). هر ۵ یا ۶ هزارتا هم با فیلتر روغن عوضش کنی، عمر سوپاپ و پیستون رو سفت بیمه کردی!`;
  }

  return `ارادت رفیق! من اوس عماد هستم، چاکر شما و راننده‌های مشتی. درباره خودروی «${carModel}» و قطعه «${partName || "لوازم مصرفی"}» تحقیق زنده داشتم. 
پایه کار اینه که رانندگی با دورهای متناسب و چک کردن مداوم سطح مایعات رادیاتور و ترمز، خرج ماشینت رو تا نصف کاهش میده. برای خرید مطمئن، پیشنهاد اولم استعلام آنلاین از دیجی‌کالا و بعد مقایسه قیمت‌ها در بستر ترب هست.
سوالی درباره اجرت نصب، روشن شدن چراغ چک، صدای غیرعادی چرخ یا ترمز دارید مطرح کنید تا با هم مثل دوتا رفیق صمیمی بررسیش کنیم! دمت گرم.`;
}
...
۲. اگر قطعه‌ای یافت نشد یا در دیجی کالا موجود نبود، به سراغ نتایج دیگر وب و معتبرترین فروشگاه‌های قطعات یدکی و قطعه‌سازان در ایران (مثل ترب torob.com، یدک کالا yadakkala.com، یدک‌یاب و غیره) یا قیمت صنف بازار چراغ‌برق تهران برو.
...
app.post("/api/car-parts/chat", async (req, res) => {
  ...
  const chatInstruction = `تو یک مکانیک فوق‌العاده با سلیقه، کاربلد و با حوصله ایرانی هستی که به زبان سنتی کارگاهی اما بسیار محترمانه و صمیمی به رانندگان کمک می‌کنی. نام تخصص تو "اوس عماد" است. اطلاعات بسیار کاملی درباره خودروها، قطعات یدکی، لوازم مصرفی، نحوه فیلتر کردن قطعه تقلبی از اصلی، هزینه‌های اجرت تعویض در صنف مکانیک‌های ایران داری. 
اگر کاربر درباره قیمت و خرید قطعه‌ای سوالی پرسید یا استعلام قیمت کارخانه و بازار خواست، با ابزار گوگل سرچ که در اختیار داری قیمت را مستقیماً از سایت دیجی‌کالا (digikala.com) استعلام کن و اگر موجود نداشت، از بهترین فروشگاه‌های لوازم یدکی ایران (مثل ترب torob.com، یدک کالا یا یدک‌یاب) قیمت معتبر بده.
همیشه راهنمایی‌های عملی، ترفندهای پول‌سازی یا کم‌کردن خرج بیخودی، و علائم دقیق نقص فنی را بده. کاربر درباره خودرو "${carModel}" و احتمالاً قطعه خاص "${partName || "عمومی"}" سوالی دارد. به زبان فارسی پاسخ بده و از اصطلاحات شیرین مکانیک‌های ایرانی هم با احتیاط و جذاب استفاده کن (مثلا آب‌بندی، سه کار کردن، رگلاژ، واشر زدن، یاتاقان، کمپرس سیلندر و غیره).`;

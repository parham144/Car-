...
export const OusEmadChat: React.FC<OusEmadChatProps> = ({
  ...
  useEffect(() => {
    setMessages([
      {
        sender: "mechanic",
        text: `یا علی مدد رفیق! من «اوس عماد» هستم، در خدمت شما برای گپ و گفت فنی درباره خودروی ${carModel}. هر سوالی درباره لوازم یدکی، کیفیت قطعات بازار چراغ‌برق، دستمزد مکانیک یا عیب‌یابی مشکوک داری بپرس تا کلک کار رو بکنیم!`,
  ...
  const handleSendMessage = async (textToSend: string) => {
    ...
    try {
      ...
      setMessages((prev) => [
        ...
        {
          sender: "mechanic",
          text: data.reply || "مخلصیم داداش! موتور ماشینت چیزیش نیست، یه کم سوالت رو واضح‌تر بگو پاسخ بدم.",
          timestamp: new Date().toLocaleTimeString("fa-IR"),
        },
      ]);
    } catch (err: any) {
